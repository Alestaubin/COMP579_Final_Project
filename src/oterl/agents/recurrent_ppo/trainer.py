...
class Trainer:
    ...
    def __init__(self, config, env, writer_path=None,save_path=None):
        ...
        try:
            ...
            timestamp = str( "RPPO2_" + time.strftime('%Y-%m-%d_%H-%M-%S'))
            self.save_dir = os.path.join(self.save_path, timestamp)
    ...
    def train(self, write_data=True):
        ...
        while training:
            ...
            for _ in tqdm(range(self.config["num_epochs"]), desc="Training epochs"):
    ...
    def _save_model(self):
        ...
        """
        # Save model
        torch.save(self.model.state_dict(), os.path.join(self.save_dir, f"model_{self.data['step']}.pt"))

        # Save stats
        with open(os.path.join(self.save_dir, f"stat_{self.data['step']}.json"), "w") as f:
