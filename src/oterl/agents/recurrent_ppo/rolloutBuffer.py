...
class RolloutBuffer:
  ...
  def __init__(self, config, state_size, action_size) -> None:
    ...
    self.batch = {
      ...
      'p_states': torch.zeros((self.num_game_per_batch, self.max_eps_length, self.hidden_size)),
      'v_states': torch.zeros((self.num_game_per_batch, self.max_eps_length, self.hidden_size)),
  ...
  def add_data(
    self, state, p_state, v_state, action, value, reward, done, valid_action, prob, policy
  ):
    """Add data to rollout buffer"""
    self.batch['states'][self.game_count][self.step_count] = state
    self.batch['p_states'][self.game_count][self.step_count] = p_state
    self.batch['v_states'][self.game_count][self.step_count] = v_state
  ...
  def cal_advantages(self, gamma: float, gae_lambda: float):
    """
    Overview:
        Calculate GAE.
  ...
  def prepare_batch(self):
    ...
    samples = {
      ...
      'loss_mask': torch.ones(
        (self.num_game_per_batch, self.max_eps_length), dtype=torch.bool
      ),  # The loss mask is used for masking the padding while computing the loss function.
      'p_states': self.batch['p_states'],
      'v_states': self.batch['v_states'],
    }
    ...
    self.flat_sequence_indices = self._arange_data_to_sequences(
      torch.arange(0, self.num_game_per_batch * self.max_eps_length).reshape(
        (self.num_game_per_batch, self.max_eps_length)
      )
    )[0]
    self.flat_sequence_indices = np.array(
      [t.numpy() for t in self.flat_sequence_indices], dtype=object
    )
  ...
  def mini_batch_generator(self):
    ...
    num_sequences_per_batch = [
      num_sequences_per_batch
    ] * self.n_mini_batches  # Arrange a list that determines the sequence count for each mini batch
