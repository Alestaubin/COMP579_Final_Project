...
class Agent():
    ...
    def play(self, state, testing=False):
        ...
        # holdings_pct is the current holdings divided by the required holdings
        holdings_pct = obs[0][0]
        # add a penalty for not holding the required amount
        penalty =  10 * (1-obs[0][0])**2 
        reward -= penalty
